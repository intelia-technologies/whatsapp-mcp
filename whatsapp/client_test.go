...
func TestResolveReactionTarget(t *testing.T) {
	incomingGroup := &storage.Message{
		ChatJID:   "120363405998813710@g.us",
		SenderJID: "34649815107@s.whatsapp.net",
		IsFromMe:  false,
	}
	ownGroup := &storage.Message{
		ChatJID:   "120363405998813710@g.us",
		SenderJID: "34690698532:27@s.whatsapp.net",
		IsFromMe:  true,
	}
	...
	chat, sender, err = resolveReactionTarget(nil, "MSG3", "34666438493-1351846975@g.us", "34652668320@s.whatsapp.net")
	if err != nil {
		t.Fatalf("explicit target: unexpected error %v", err)
	}
	if chat.String() != "34666438493-1351846975@g.us" || sender.String() != "34652668320@s.whatsapp.net" {
